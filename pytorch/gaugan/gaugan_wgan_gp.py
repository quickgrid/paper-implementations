...
class ImageEncoder(nn.Module):
    def __init__(
        ...
        channel_in = [3, 64, 128, 128, 256, 512]
        channel_out = [64, 128, 128, 256, 512, 512]
...
class Trainer:
    ...
    def get_gradient_penalty(
        ...
        epsilon = torch.randn((batch_size, 1, 1, 1), device=self.device).repeat(1, c, h, w)
        interpolated_images = real * epsilon + fake * (1 - epsilon)

        mixed_scores, multiscale_features = self.discriminator(segmentation_image, interpolated_images)
            ...
            with tqdm(self.train_loader) as tqdm_train_loader:
                for batch_idx, (real_image, segmentation_image, segmentation_label) in enumerate(tqdm_train_loader):
                    ...
                    for i in range(self.disc_iterations):
                        ...
                        gp = 10 * self.get_gradient_penalty(real_image, generated_image, segmentation_image)
                        discriminator_loss = (
                                -(torch.mean(real_pred) - torch.mean(fake_pred.detach())) + gp
                        )
